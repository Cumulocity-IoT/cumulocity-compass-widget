    ...
    template:
        ...
        <svg version="1.0" viewBox="0 0 470 470" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
            ...
            <path
                    ...
                    transform="translate(227,227) rotate(0)"
                    d="M 0,90 
      V -90
...
export class CompassWidget implements OnDestroy {

    private widgetConfiguration;
    private realtimeMeasurements$;
    private currentCompassPosition = {property: 0};
    ...
    ngOnDestroy(): void {
        // Unsubscribe from a previous subscription (if any)
        if( this.realtimeMeasurements$ !== undefined ) {
            this.realtime.unsubscribe(this.realtimeMeasurements$);
        }
    }

    private async updateForDeviceChange(config) {
        // Get the configuration details
        const device_id = _.get(this.widgetConfiguration, 'device.id');
        const fragment  = _.get(this.widgetConfiguration, 'measurement.fragment');
        const series    = _.get(this.widgetConfiguration, 'measurement.series');

        // Get the last measurement
        const now = moment();
        const filter = {
            source:config.device.id,
            dateFrom:'1970-01-01',
            dateTo: now.add(1, 'days').format('YYYY-MM-DDTHH:mm:ssZ'),
            pageSize:1, revert:true
        };

        const {data, res, paging} = await this.measurementService.list(filter);
        if (data[0] == undefined || !_.has(data[0], `${fragment}.${series}`)) {
            return;
        }
        this.moveCompassNeedle(_.get( data[0], `${fragment}.${series}.value`));

        // Subscribe to real-time updates
        this.realtimeMeasurements$ = this.realtime.subscribe(`/measurements/${device_id}`,
            ({ 
                data: {
                    data: measurementObject
                }
            }) => {
                if (!_.has(measurementObject, `${fragment}.${series}`)) {
                    return;
                }
                // console.log(`Setting needle to ${_.get( measurementObject, `${fragment}.${series}.value`)}`);
                this.moveCompassNeedle( _.get(measurementObject, `${fragment}.${series}.value`) )
            })
    }

    moveCompassNeedle(heading) {
        // we use a svg transform to move to correct orientation and location
        const translateValue = "translate(227,227)";
        const to = {property: heading};

        // stop the current animation and run to the new one
        $(this.currentCompassPosition).stop().animate(to, {
            duration: 500,
            step: function() {
                $("#compass_needle").attr("transform", translateValue + " rotate(" + this.property + ")")
            }
        });
    }
